...
type ContentAPIComponentValue<T extends Attribute> =
  T extends ComponentAttribute<infer U, infer R>
    ? WithID &
        (R extends true ? GetAttributesValues<U>[] : GetAttributesValues<U>)
    : never;

// Custom GetMediaAttributeValue implementation for the content api
type ContentAPIMediaValue<T extends Attribute> = T extends MediaAttribute<
  infer R
>
  ? R extends true
    ? DataWrapper
...
type ContentAPIDynamicZoneValue<T extends Attribute> =
  T extends DynamicZoneAttribute<infer C>
    ? Array<
...
export type GetAllowedAttributesKey<T extends utils.SchemaUID> =
  GetAttributes<T> extends infer A
...
type FlatCollectionType<T> = T extends
  | ResponseCollection<infer K>
  | Omit<ResponseCollection<infer K>, "meta">
  ? {
      [Key in keyof DataWrapper<K>["attributes"]]: FlatCollectionType<
        DataWrapper<K>["attributes"][Key]
      >;
    }
  : T;

export type FlatCollection<T extends utils.SchemaUID> = FlatCollectionType<
